...
const App = () => {
      ...
      const [tempConnection, setTempConnection] =
            useState<TempConnection | null>(null);
      const [mousePos, setMousePos] = useState({ x: 0, y: 0 });

      // Modals & Menus
      const [relMenu, setRelMenu] = useState<{
            id: string;
            x: number;
            y: number;
      } | null>(null);
      const [warningModal, setWarningModal] = useState<WarningData | null>(
            null,
      );
      ...
      const { viewTables, viewRelationships } = useMemo(() => {
            ...
            relationships.forEach((rel) => {
                  if (rel.type === "N:M") {
                        ...
                        if (sourceTable && targetTable) {
                              ...
                              const midX =
                                    (sourceTable.x + targetTable.x) / 2 +
                                    TABLE_WIDTH / 2 -
                                    TABLE_WIDTH / 2; // Center align
                              const midY = (sourceTable.y + targetTable.y) / 2;

                              // 2. Derive Columns from Source/Target PKs
                              const sourcePks = sourceTable.columns.filter(
                                    (c) => c.isPk,
                              );
                              const targetPks = targetTable.columns.filter(
                                    (c) => c.isPk,
                              );

                              const newColumns: Column[] = [];

                              const getVirtualName = (
                                    colId: string,
                                    defaultName: string,
                              ) => {
                                    if (
                                          rel.virtualColNames &&
                                          rel.virtualColNames[colId]
                                    ) {
            ...
            return {
                  viewTables: physicalTables,
                  viewRelationships: physicalRels,
            };
      ...
      const propagateColumnChanges = (
            ...
            const childRels = relationships.filter(
                  (r) =>
                        r.fromTable === parentTableId &&
                        r.fromCol === parentColId,
            );
            if (childRels.length === 0) return;

            setTables((prevTables) =>
                  prevTables.map((t) => {
                        const relevantRel = childRels.find(
                              (r) => r.toTable === t.id,
                        );
                              ...
                              columns: t.columns.map((col) => {
                                    if (col.id === relevantRel.toCol) {
                                          return {
                                                ...
                                                isNullable:
                                                      updatedParentCol.isIdentity ||
                                                      !updatedParentCol.isNullable
                                                            ? false
                                                            : true,
                                          };
                                    }
      ...
      const completeConnectionToNewColumn = (
            ...
            if (isConnecting && tempConnection) {
                  ...
                  const sourceTable = tables.find(
                        (t) => t.id === tempConnection.sourceTableId,
                  );
                  const sourceCol = sourceTable?.columns.find(
                        (c) => c.id === tempConnection.sourceColId,
                  );
                  const targetTable = tables.find(
                  ...
                  const newCol: Column = {
                        ...
                        isFk: true,
                        isNullable:
                              sourceCol.isIdentity || !sourceCol.isNullable
                                    ? false
                                    : true,
                        isUnique: false,
                        isIdentity: false,
      ...
      const handleTablePointerDown = (e: React.PointerEvent, id: string) => {
            ...
            const isLocked =
                  globalEditable ||
                  (targetTable && targetTable.isManuallyEditable);

            if (isLocked) {
                  // Just select it, don't drag
                  setSelectedId(id);
                  setIsPropertiesPanelOpen(true); // Always open panel when locked (click to edit)
                  return;
            }
      ...
      const addTable = () => {
            ...
            const containerWidth =
                  window.innerWidth - (selectedId ? sidebarWidth : 0) - 224; // approx sidebar widths
      ...
      const promoteVirtualTable = (virtId: string) => {
            ...
            const connectedRels = viewRelationships.filter(
                  (r) => r.toTable === virtId,
            );
      ...
      const updateRelName = (id: string, name: string) => {
            setRelationships(
                  relationships.map((r) => (r.id === id ? { ...r, name } : r)),
            );
      };
                                                            ...
                                                            (rel) => {
                                                                  ...
                                                                  const pts =
                                                                        getConnectorPoints(
                                                                              rel,
                                                                              viewTables,
                                                                        ); // Use viewTables for calculation
                                                                  const midpoint =
                                                                        getCurveMidpoint(
                                                                              rel,
                                                                              viewTables,
                                                                  ...
                                                                  return (
                                                                        ...
                                                                        >
                                                                              ...
                                                                              {/* Cardinality Labels */}
                                                                              {viewOptions.showCardinalityNumeric &&
                                                                                    pts && (
                                                                                          <>
                                                                                                <text
                                                                                                      x={
