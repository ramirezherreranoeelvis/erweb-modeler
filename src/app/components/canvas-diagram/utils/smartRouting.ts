...
export const getSmartTableRoute = (source: Table, target: Table, rel: Relationship): Point[] => {
      const sRect: Rect = { x: source.x, y: source.y, w: TABLE_WIDTH, h: getTableHeight(source) };

      // --- SPECIAL CASE: SELF REFERENCE ---
      if (source.id === target.id) {
            // Default to Right-Right loop if sides not manually overridden to something incompatible
            const startSide = rel.sourceSide || 'right';
            const endSide = rel.targetSide || 'right';

            if (startSide === 'right' && endSide === 'right') {
                  const rightX = sRect.x + sRect.w;
                  const topY = sRect.y + 40; // Start somewhat near top (PK area)
                  const bottomY = sRect.y + sRect.h - 40; // End somewhat near bottom
                  const buffer = 40;

                  return [
                        { x: rightX, y: topY },
                        { x: rightX + buffer, y: topY },
                        { x: rightX + buffer, y: bottomY },
                        { x: rightX, y: bottomY },
                  ];
            }

            // If user forces Left-Left
            if (startSide === 'left' && endSide === 'left') {
                  const leftX = sRect.x;
                  const topY = sRect.y + 40;
                  const bottomY = sRect.y + sRect.h - 40;
                  const buffer = 40;

                  return [
                        { x: leftX, y: topY },
                        { x: leftX - buffer, y: topY },
                        { x: leftX - buffer, y: bottomY },
                        { x: leftX, y: bottomY },
                  ];
            }

            // Fallback for Top/Bottom or mixed on self-ref will be handled by standard logic below
            // but standard logic treats source==target as overlapping
      }
      ...
      const xOverlap = Math.max(
            0,
            Math.min(sRect.x + sRect.w, tRect.x + tRect.w) - Math.max(sRect.x, tRect.x),
      );
      // Vertical Overlap (height of intersection on Y axis)
      const yOverlap = Math.max(
            0,
            Math.min(sRect.y + sRect.h, tRect.y + tRect.h) - Math.max(sRect.y, tRect.y),
      );
      ...
      const validZVertical =
            isOppositeVertical &&
            ((startSide === 'bottom' && startPt.y < endPt.y) ||
                  (startSide === 'top' && startPt.y > endPt.y));
      const validZHorizontal =
            isOppositeHorizontal &&
            ((startSide === 'right' && startPt.x < endPt.x) ||
                  (startSide === 'left' && startPt.x > endPt.x));
