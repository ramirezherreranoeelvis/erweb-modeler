...
import { DbEngine, getCanonicalType } from './dbDataTypes';
                  ...
                  if (sourceTable && targetTable) {
                        ...
                        sourcePks.forEach((pk) => {
                              const col = newColumns.find(
                                    (c) => c.id === `${intersectId}_src_${pk.id}`,
                              );
                              if (col) {
                                    physRels.push({
                                          id: generateId(),
                        ...
                        targetPks.forEach((pk) => {
                              const col = newColumns.find(
                                    (c) => c.id === `${intersectId}_tgt_${pk.id}`,
                              );
                              if (col) {
                                    physRels.push({
                                          id: generateId(),
      ...
      physTables.forEach((table) => {
            ...
            table.columns.forEach((col) => {
                  ...
                  if (
                        isPostgres &&
                        upperType === 'ENUM' &&
                        postgresEnumTypes.has(`${table.id}:${col.id}`)
                  ) {
                  ...
                  if (col.isIdentity) {
                        if (isMysql) {
                              ...
                              const isSerialType = ['SERIAL', 'BIGSERIAL', 'SMALLSERIAL'].includes(
                                    upperType,
                              );
                              if (!isSerialType) {
            ...
            if (pkCols.length > 0) {
                  if (isMysql) {
                        lines.push(
                              `  PRIMARY KEY (${pkCols.map((c) => `${openQ}${c}${closeQ}`).join(', ')})`,
                        );
                  } else {
                        lines.push(
