                        ...
                        lines.forEach((line) => {
                              ...
                              if (parts.length >= 2) {
                                    ...
                                    let restOfLine = line.substring(
                                          line.toUpperCase().indexOf(parts[1].toUpperCase()) +
                                                parts[1].length,
                                    );
                                    // If we grabbed length separately (e.g. "DECIMAL (10,2)"), advance past it
                                    if (restOfLine.trim().startsWith('(')) {
                                          restOfLine = restOfLine.substring(
                                                restOfLine.indexOf(')') + 1,
                                          );
                                    }
                  ...
                  if (childTable) {
                        ...
                        lines.forEach((line) => {
                              const refMatch = line.match(
                                    /(\w+).*REFERENCES\s+([^\s(]+)\s*\(([^)]+)\)/i,
                              );
                              if (refMatch) {
                                    const colName = cleanIdentifier(refMatch[1]);
                                    let parentTblRaw = refMatch[2];
                                    if (parentTblRaw.includes('.'))
                                          parentTblRaw = parentTblRaw.split('.').pop()!;
