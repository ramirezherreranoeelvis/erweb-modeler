...
export const shouldShowLength = (type: string): boolean => {
  ...
  const noLengthPatterns = [
    ...
    'HIERARCHYID', 'UNIQUEIDENTIFIER', 'ROWVERSION', 'IMAGE', 'SQL_VARIANT', 'NTEXT', 'TEXT', 'MONEY', 'SMALLMONEY', 'REAL', 'FLOAT', 'INT', 'BIGINT', 'SMALLINT', 'TINYINT'
  ];

  // Specific check for BIT: often BIT(1) but can be BIT(64). Usually optional default 1.
  // We'll let it show length but not require it.
...
export const isLengthRequired = (type: string): boolean => {
  ...
  // These types generally require a length/precision specification to be valid or useful
  return [
