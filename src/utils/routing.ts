...
export const getConnectorPoints = (r: Relationship, tables: Table[], connectionMode: 'column' | 'table' = 'column') => {
  ...
  // The 'top'/'bottom' override logic is handled nicely in Table mode above
...
export const getRoutePoints = (
  ...
  if (style === 'orthogonal') {
    ...
    if (isCShape) {
      ...
      // In Table Mode, we might want to prioritize MidY split if vertical distance is greater
      const dx = Math.abs(p1x - p2x);
      const dy = Math.abs(p1y - p2y);
      
      // If purely vertical stacking (often happens in table mode), do a Z-shape via Y
      ...
      // Default horizontal S-Shape logic checking for table collisions
      const startTable = tables.find((t) => t.id === r.fromTable);
      const endTable = tables.find((t) => t.id === r.toTable);
