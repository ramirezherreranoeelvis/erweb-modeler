...
export const parseSQL = (sqlInput: string, engine: DbEngine): { tables: Table[], relationships: Relationship[] } => {
    ...
    if (createTableMatch) {
      ...
      if (firstParen > -1 && lastParen > -1) {
        ...
        lines.forEach(line => {
          ...
          const pkConstraintMatch = line.match(/CONSTRAINT\s+([^\s]+)\s+PRIMARY\s+KEY(?:\s+(?:CLUSTERED|NONCLUSTERED))?\s*\(([^)]+)\)/i);
          if (pkConstraintMatch) {
            const cols = pkConstraintMatch[2].split(',').map(c => cleanIdentifier(c.trim().split(/\s+/)[0]));
            pkColumns.push(...cols);
            return;
          }
          ...
          if (parts.length >= 2) {
             ...
             if (/^(INDEX|KEY|CONSTRAINT|UNIQUE|FOREIGN|FULLTEXT)$/i.test(cleanIdentifier(rawColName))) {
               return; 
             }
             ...
             const isIdentity = restOfLine.includes('AUTO_INCREMENT') || restOfLine.includes('IDENTITY');
  ...
  // Foreign Key Parsing
  statements.forEach(stmt => {
    const alterFkMatch = stmt.match(/ALTER\s+TABLE\s+([^\s]+)\s+.*FOREIGN\s+KEY\s*\(([^)]+)\)\s*REFERENCES\s+([^\s]+)\s*\(([^)]+)\)/i);
