...
export const useSchemaData = (viewMode: string) => {
  ...
  const { viewTables, viewRelationships } = useMemo(() => {
    ...
    nmRels.forEach((rel) => {
      ...
      if (sourceTable && targetTable) {
        ...
        const getVirtualName = (colId: string, defaultName: string) =>
          rel.virtualColNames && rel.virtualColNames[colId]
        ...
        targetPks.forEach((pk) => {
          ...
          if (newColumns.some((c) => c.name === defaultName))
        ...
        sourcePks.forEach((pk) => {
          ...
          if (col)
            physicalRels.push({
              ...
              name: `fk_${sourceTable.name}_${pk.name}`,
        ...
        targetPks.forEach((pk) => {
          ...
          if (col)
            physicalRels.push({
              ...
              name: `fk_${targetTable.name}_${pk.name}`,
    ...
    const finalLogicalRels = logicalRels.filter(
      (r) => !hiddenTableIds.has(r.toTable) && !hiddenTableIds.has(r.fromTable),
    );
  ...
  const calculatePromotedState = (virtId: string) => {
    ...
    targetPks.forEach((pk) => {
      ...
      let def = pk.name;
      if (newColumns.some((c) => c.name === def))
        def = `${targetTable.name.toLowerCase()}_${pk.name}`;
      newColumns.push({
        ...pk,
        id: generateId(),
        name: getVirtualName(virtColId, def),
        isPk: true,
        isFk: true,
        isIdentity: false,
        isNullable: false,
    ...
    sourcePks.forEach((pk, i) => {
      newRels.push({
        id: generateId(),
        name: `fk_${sourceTable.name}_${newTable.name}`.toLowerCase(),
        fromTable: sourceTable.id,
        fromCol: pk.id,
        toTable: newTableId,
        toCol: newColumns[i].id,
        type: '1:N',
      });
    });
    targetPks.forEach((pk, i) => {
      newRels.push({
        id: generateId(),
        name: `fk_${targetTable.name}_${newTable.name}`.toLowerCase(),
        fromTable: targetTable.id,
        fromCol: pk.id,
        toTable: newTableId,
        toCol: newColumns[sourcePks.length + i].id,
        type: '1:N',
      });
    });
  ...
  const actions = {
    ...
    deleteTable: (id: string | null) => {
      ...
      setTables((prevTables) => {
        ...
        return remainingTables.map((t) => {
          ...
          const incomingRelsRemoved = relsRemoving.filter(
            (r) => r.toTable === t.id && r.fromTable === id,
          );
            ...
            columns: t.columns.map((c) => {
              if (colsToCheck.has(c.id)) {
                ...
                const otherRels = relationships.filter(
                  (r) => r.toTable === t.id && r.toCol === c.id && r.fromTable !== id,
                );
    ...
    updateColumn: (tableId: string, colId: string, field: string, value: any) => {
      ...
      if (field === 'name') {
        ...
        if (
          table &&
          table.columns.some((c) => c.id !== colId && c.name.toLowerCase() === value.toLowerCase())
        )
          return;
      }
    ...
    updateRelName: (id: string, name: string) => {
      if (id.startsWith('virt_rel_')) {
        ...
        if (promo) {
          const { newTable, newRels, oldRelId } = promo;
          const targetRelIndex = newRels.findIndex(
            (r) => r.fromTable === virtRel.fromTable && r.fromCol === virtRel.fromCol,
          );
          if (targetRelIndex !== -1) {
              ...
              return {
                ...
                columns: t.columns.map((c) => {
                  if (c.id === rel.toCol) {
                    ...
                    const otherRels = relationships.filter(
                      (r) => r.id !== relId && r.toTable === t.id && r.toCol === c.id,
                    );
                    if (otherRels.length === 0) {
                      return { ...c, isFk: false };
                    }
    ...
    deleteRel: (id: string) => {
      if (id.startsWith('virt_rel_')) {
        ...
        if (promo) {
          ...
          const filteredRels = newRels.filter(
            (r) => !(r.fromTable === virtRel.fromTable && r.fromCol === virtRel.fromCol),
          );
          const deletedRel = newRels.find(
            (r) => r.fromTable === virtRel.fromTable && r.fromCol === virtRel.fromCol,
          );
          ...
          if (deletedRel) {
            finalTable = {
              ...newTable,
              columns: newTable.columns.map((c) =>
                c.id === deletedRel.toCol ? { ...c, isFk: false } : c,
              ),
            };
          }
        ...
        setTables((prev) =>
          prev.map((t) => {
            if (t.id === rel.toTable) {
              ...
              const otherRels = relationships.filter(
                (r) => r.id !== id && r.toTable === t.id && r.toCol === rel.toCol,
              );
    ...
    resetRelRouting: (id: string) => {
      setRelationships((prev) =>
        prev.map((r) =>
          r.id === id
            ? { ...r, sourceSide: undefined, targetSide: undefined, controlPoints: [] }
            : r,
        ),
      );
    },
