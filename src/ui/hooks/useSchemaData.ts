...
export const useSchemaData = (viewMode: string, dbEngine: DbEngine) => {
  ...
  const actions = useMemo(() => ({
    ...
    applyConnection: (sourceTId: string, sourceCId: string, targetTId: string, targetCId: string) => {
        ...
        if (sourceCol && targetCol) {
            ...
            const relName = `fk_${sourceTable?.name}_${sourceCol.name}_${targetTable?.name}_${targetCol.name}`.toLowerCase();
    ...
    resetRelRouting: (id: string) => {
        setRelationships((prev) =>
            prev.map((r) => (r.id === id ? { ...r, controlPoints: [], sourceSide: undefined, targetSide: undefined } : r))
        );
    },
