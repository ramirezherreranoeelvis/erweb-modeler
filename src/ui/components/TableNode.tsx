...
const TableNode: React.FC<TableNodeProps> = ({
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onPointerDown={(e) => {
              e.preventDefault();
              e.stopPropagation();
              e.nativeEvent.stopImmediatePropagation(); // Ensure this doesn't trigger table selection/drag
            }}
            onClick={(e) => {
              e.stopPropagation();
              onUpdateTable(table.id, 'isManuallyEditable', !table.isManuallyEditable);
            }}
            className={`p-1.5 rounded-md transition-colors ${table.isManuallyEditable ? 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300' : 'text-slate-300 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
            title={
        ...
        {table.columns.map((col, index) => (
          ...
          >
            ...
            <div
              ...
              onDoubleClick={(e) => {
                e.stopPropagation();
                startEditingColumn(col.id, 'name', col.name);
              }}
            >
              ...
              {viewMode === 'physical' && viewOptions.showTypes && (
                <>
                  ...
                  <div
                    onDoubleClick={(e) => {
                      e.stopPropagation();
                      startEditingColumn(col.id, 'type', col.type);
                    }}
                  >
      ...
      <div
        className={`absolute -bottom-3 left-1/2 -translate-x-1/2 z-20 transition-opacity duration-200 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover/table:opacity-100'}`}
        onMouseDown={(e) => e.stopPropagation()}
        onClick={(e) => {
          e.stopPropagation();
