...
const DiagramCanvas: React.FC<DiagramCanvasProps> = ({
  ...
  const handleControlPointPointerDown = (e: React.PointerEvent, relId: string, routeIndex: number, currentX: number, currentY: number) => {
    e.stopPropagation();
    if (document.activeElement instanceof HTMLElement) document.activeElement.blur();
    setRelMenu(null); setIsPropertiesPanelOpen(false); setSelectedId(null);
    if (setSelectedTableIds) setSelectedTableIds(new Set());
    if (!mainRef.current) return;
    ...
    let bestIndex = -1, minD = Infinity;
    newCPs.forEach((p, idx) => {
      const d = Math.hypot(p.x - currentX, p.y - currentY);
      if (d < minD) { minD = d; bestIndex = idx; }
    });
    if (bestIndex === -1) bestIndex = Math.max(0, Math.min(routeIndex - 1, newCPs.length - 1));
    setSelectedCP({ relId, index: bestIndex });
    setDragInfo({ isDragging: true, offset: { x: clickX - currentX, y: clickY - currentY }, targetId: `cp:${relId}:${bestIndex}` });
    (e.currentTarget as Element).setPointerCapture(e.pointerId);
  };

  const handleSegmentPointerDown = (e: React.PointerEvent, relId: string, segmentStartIndex: number, isVertical: boolean) => {
    e.stopPropagation(); e.preventDefault(); setRelMenu(null); setSelectedCP(null);
    if (!mainRef.current) return;
    materializeRoute(relId);
    const canvasRect = mainRef.current.getBoundingClientRect();
    const clickX = (e.clientX - canvasRect.left - pan.x) / zoom;
    const clickY = (e.clientY - canvasRect.top - pan.y) / zoom;
    setDragInfo({ isDragging: true, offset: { x: clickX, y: clickY }, targetId: `seg:${relId}:${segmentStartIndex}:${isVertical ? 'v' : 'h'}` });
    (e.currentTarget as Element).setPointerCapture(e.pointerId);
  };
  ...
  const handleRelDoubleClick = (e: React.MouseEvent, relId: string) => {
    ...
    if (insertIdx >= 0 && insertIdx <= newPoints.length) newPoints.splice(insertIdx, 0, { x: clickX, y: clickY });
    else newPoints.push({ x: clickX, y: clickY });
    setRelationships((prev) => prev.map((r) => (r.id === relId ? { ...r, controlPoints: newPoints } : r)));
    setRelMenu(null);
  };
  ...
  const startConnection = (e: React.PointerEvent, tableId: string, colId: string, side: 'left' | 'right' | 'top' | 'bottom') => {
    e.stopPropagation(); setRelMenu(null); setSelectedCP(null);
  ...
  const completeConnection = (
    ...
    if (isConnecting && tempConnection) {
      ...
      if (sourceTId === targetTableId || targetTableId.startsWith('virt_') || sourceTId.startsWith('virt_')) {
        setIsConnecting(false); setTempConnection(null); return;
      }
      ...
      if (viewOptions.connectionMode === 'table') {
        const existingRel = relationships.find(r => (r.fromTable === sourceTId && r.toTable === targetTableId) || (r.fromTable === targetTableId && r.toTable === sourceTId));
        if (existingRel) { setIsConnecting(false); setTempConnection(null); return; }
        const existingTargetCol = targetTable.columns.find(c => c.name.toLowerCase() === sourceCol.name.toLowerCase());
        if (existingTargetCol) {
          setWarningModal({ isOpen: true, type: 'collision', data: { sourceTId, sourceCId, targetTId: targetTableId, targetCId: existingTargetCol.id, sourceCol, targetCol: existingTargetCol } });
        } else {
      ...
      if (!targetCol) return;
      const exists = relationships.find(r => (r.fromTable === sourceTId && r.fromCol === sourceCId && r.toTable === targetTableId && r.toCol === targetColId) || (r.toTable === sourceTId && r.toCol === sourceCId && r.fromTable === targetTableId && r.fromCol === targetColId));
      if (exists || targetCol.isFk) { setIsConnecting(false); setTempConnection(null); return; }
      ...
      const hasMismatch = !compatibleTypes || lengthMismatch || targetCol.isNullable !== expectedNullable;

      if (hasMismatch) {
        setWarningModal({ isOpen: true, type: 'integrity', data: { sourceTId, sourceCId, targetTId: targetTableId, targetCId: targetColId, sourceCol, targetCol } });
      } else {
        onApplyConnection(sourceTId, sourceCId, targetTableId, targetColId);
      }
      setIsConnecting(false); setTempConnection(null);
    }
  };
  ...
  const completeConnectionToNewColumn = (e: React.PointerEvent, targetTableId: string) => {
    if (isConnecting && tempConnection) {
      ...
      const newCol: Column = {
        id: newColId, name: newName, logicalName: newLogicalName, type: targetType, length: sourceCol.length,
        isPk: false, isFk: true, isNullable: sourceCol.isIdentity || !sourceCol.isNullable ? false : true, isUnique: false, isIdentity: false,
      };
      setTables((prev) => prev.map((t) => (t.id === targetTableId ? { ...t, columns: [...t.columns, newCol] } : t)));
      const relName = `fk_${sourceTable?.name}_${sourceCol.name}_${targetTable.name}_${newName}`.toLowerCase();
      const newRel: Relationship = { id: generateId(), name: relName, fromTable: tempConnection.sourceTableId, fromCol: tempConnection.sourceColId, toTable: targetTableId, toCol: newColId, type: '1:N' };
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (dragInfo.isDragging && dragInfo.targetId) {
      ...
      if (dragInfo.targetId.startsWith('cp:')) {
        ...
        onUpdateControlPoint(parts[1], parseInt(parts[2], 10), x - dragInfo.offset.x, y - dragInfo.offset.y);
        return;
      }
      if (dragInfo.targetId.startsWith('seg:')) {
        ...
        const updateCP = (idx: number, newCoord: number, type: 'x' | 'y') => {
          ...
          onUpdateControlPoint(relId, idx, type === 'x' ? newCoord : pt.x, type === 'y' ? newCoord : pt.y);
        };
        if (axis === 'v') { updateCP(cpIndices[0], x, 'x'); updateCP(cpIndices[1], x, 'x'); }
        else { updateCP(cpIndices[0], y, 'y'); updateCP(cpIndices[1], y, 'y'); }
        return;
      }
      if (dragInfo.targetId.startsWith('virt_')) {
        const relId = dragInfo.targetId.replace('virt_', '');
        let newX = x - dragInfo.offset.x; let newY = y - dragInfo.offset.y;
        if (viewOptions.snapToGrid) { newX = Math.round(newX / 20) * 20; newY = Math.round(newY / 20) * 20; }
        setRelationships((prev) => prev.map((r) => (r.id === relId ? { ...r, x: newX, y: newY } : r)));
        return;
      }
      ...
      if (targetTable) {
        ...
        if (viewOptions.snapToGrid) { newTx = Math.round(newTx / 20) * 20; newTy = Math.round(newTy / 20) * 20; }
        ...
        setTables((prev) => prev.map((t) => {
            ...
            if (isDraggingSelected && selectedTableIds?.has(t.id)) return { ...t, x: t.x + dx, y: t.y + dy };
            return t;
        }));
        ...
        if (viewOptions.lineStyle === 'orthogonal') {
          setRelationships((prev) => prev.map((r) => {
              ...
              const fromMoved = r.fromTable === tableId || (isDraggingSelected && selectedTableIds?.has(r.fromTable));
              const toMoved = r.toTable === tableId || (isDraggingSelected && selectedTableIds?.has(r.toTable));
              ...
              if (fromMoved && toMoved) {
                  ...
                  if (fromMoved) {
                    ...
                    const prevPt = getRoutePoints(r, tables, 'orthogonal', viewOptions.connectionMode)[0];
                    const isSegVertical = Math.abs(prevPt.x - firstCP.x) < Math.abs(prevPt.y - firstCP.y);
                    if (isSegVertical) { newCPs[0] = { ...firstCP, x: firstCP.x + dx }; modified = true; }
                    else { newCPs[0] = { ...firstCP, y: firstCP.y + dy }; modified = true; }
                  }
                  if (toMoved) {
                    ...
                    const isSegVertical = Math.abs(endAnchor.x - lastCP.x) < Math.abs(endAnchor.y - lastCP.y);
                    if (isSegVertical) { newCPs[lastIdx] = { ...lastCP, x: lastCP.x + dx }; modified = true; }
                    else { newCPs[lastIdx] = { ...lastCP, y: lastCP.y + dy }; modified = true; }
                  }
              }
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (isConnecting) { setIsConnecting(false); setTempConnection(null); }
  ...
  return (
    <main
      ...
      style={{
        ...
        cursor: isPanning ? 'grabbing' : isConnecting ? 'crosshair' : viewOptions.interactionMode === 'select' ? 'default' : dragInfo.isDragging ? 'grabbing' : 'grab',
      }}
    >
      <div className="absolute top-0 left-0 w-full h-full origin-top-left" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
        <svg className="absolute top-0 left-0 w-[10000px] h-[10000px] pointer-events-none z-0" style={{ transform: 'translate(-5000px, -5000px)' }}>
          <defs><CardinalityMarkers theme={theme} /></defs>
          <g transform="translate(5000, 5000)">
            {viewRelationships.map((rel) => {
              ...
              let startLabel = '1', endLabel = 'N';
              if (rel.type === '1:1') { endM = 'url(#oneEnd)'; endLabel = '1'; }
              if (rel.type === '1:0..N') { endM = 'url(#zeroManyEnd)'; endLabel = '0..1..N'; }
              if (rel.type === '1:0..1') { endM = 'url(#zeroOneEnd)'; endLabel = '0..1'; }
              if (rel.type === 'N:M') { startM = 'url(#manyStart)'; endM = 'url(#manyEnd)'; startLabel = 'N'; endLabel = 'M'; }
              if (rel.type === 'N:1') { startM = 'url(#manyStart)'; endM = 'url(#oneEnd)'; startLabel = 'N'; endLabel = '1'; }
              if (!viewOptions.showCardinality) { startM = undefined; endM = undefined; }
              ...
              const charWidth = 6; const padding = 16;
              ...
              const getLabelProps = (x: number, y: number, table?: Table) => {
                ...
                return { x: x + (x > centerX ? 10 : -10), y: y - 5, anchor: x > centerX ? 'start' : 'end' };
              };
              ...
              return (
                ...
                >
                  <path d={calculatePath(rel, viewTables, viewOptions.lineStyle, viewOptions.connectionMode)} stroke="transparent" strokeWidth="20" fill="none" />
                  <path d={calculatePath(rel, viewTables, viewOptions.lineStyle, viewOptions.connectionMode)} stroke={isSelected ? '#2563eb' : strokeColor} strokeWidth={isSelected ? '2.5' : '1.5'} fill="none" markerStart={startM} markerEnd={endM} strokeDasharray={dashArray} className="transition-colors duration-200" />
                  {areDotsVisible && routePoints.length > 2 && routePoints.map((p1, idx) => {
                      ...
                      return <rect key={`seg-${idx}`} x={midX - (isVertical ? 4 : 8)} y={midY - (isVertical ? 8 : 4)} width={isVertical ? 8 : 16} height={isVertical ? 16 : 8} rx={2} fill="#3b82f6" stroke="black" strokeWidth="1" className="cursor-move hover:fill-blue-400" onPointerDown={(e) => handleSegmentPointerDown(e, rel.id, idx, isVertical)} onClick={(e) => e.stopPropagation()} />;
                    })}
                  {areDotsVisible && routePoints.map((cp, idx) => {
                      if (idx === 0 || idx === routePoints.length - 1) return null;
                      const manualIndex = idx - 1;
                      const isPtSelected = selectedCP?.relId === rel.id && selectedCP?.index === manualIndex;
                      return (
                        <g key={`${rel.id}-pt-${idx}`}>
                          <circle cx={cp.x} cy={cp.y} r={isPtSelected ? 6 : 4} fill={isPtSelected ? '#f97316' : '#10b981'} stroke="white" strokeWidth="1.5" style={{ transformBox: 'fill-box', transformOrigin: 'center' }} className="cursor-move hover:scale-125 transition-transform drop-shadow-md" onPointerDown={(e) => handleControlPointPointerDown(e, rel.id, idx, cp.x, cp.y)} onClick={(e) => e.stopPropagation()} onDoubleClick={(e) => { e.stopPropagation(); if (rel.controlPoints && rel.controlPoints.length > 0) onDeleteControlPoint(rel.id, manualIndex); }} />
                          {isPtSelected && (
                            <g className="md:hidden cursor-pointer hover:opacity-80" onClick={(e) => { e.stopPropagation(); onDeleteControlPoint(rel.id, manualIndex); setSelectedCP(null); }} transform={`translate(${cp.x + 10}, ${cp.y - 12})`}>
                              <rect width="20" height="20" rx="4" fill="#ef4444" stroke="white" strokeWidth="1" />
                              <g transform="translate(4, 4) scale(0.6)"><path d="M3 6h18M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2M10 11v6M14 11v6" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></g>
                            </g>
                          )}
                        </g>
                      );
                    })}
                  {viewOptions.showCardinalityNumeric && pts && ( <> <text x={startLabelProps.x} y={startLabelProps.y} textAnchor={startLabelProps.anchor as any} className="text-[10px] font-bold fill-slate-500 dark:fill-slate-400 pointer-events-none select-none">{startLabel}</text> <text x={endLabelProps.x} y={endLabelProps.y} textAnchor={endLabelProps.anchor as any} className="text-[10px] font-bold fill-slate-500 dark:fill-slate-400 pointer-events-none select-none">{endLabel}</text> </> )}
                  {viewOptions.showRelationshipNames && rel.name && ( <g transform={`translate(${midpoint.x}, ${midpoint.y})`}> <rect x={-(labelWidth / 2)} y="-9" width={labelWidth} height="18" rx="4" fill={theme === 'dark' ? '#1e293b' : '#f8fafc'} stroke={theme === 'dark' ? '#475569' : '#cbd5e1'} strokeWidth="1" className="shadow-sm" /> <text x="0" y="3" textAnchor="middle" className={`text-[10px] font-mono font-bold pointer-events-none select-none ${isSelected ? 'fill-blue-600 dark:fill-blue-400' : 'fill-slate-600 dark:fill-slate-300'}`}>{rel.name}</text> </g> )}
                </g>
              );
            })}
            {isConnecting && tempConnection && ( <path d={`M ${tempConnection.startX} ${tempConnection.startY} L ${mousePos.x / zoom} ${mousePos.y / zoom}`} stroke="#3b82f6" strokeWidth="2" strokeDasharray="5,5" fill="none" markerEnd="url(#oneManyEnd)" /> )}
          </g>
        </svg>
        {viewTables.map((table) => ( 
            <TableNode 
                ...
                table={table} 
                tables={viewTables} 
                isSelected={selectedId === table.id || (selectedTableIds ? selectedTableIds.has(table.id) : false)} 
                viewMode={viewMode} 
                viewOptions={viewOptions} 
      ...
      <div className="absolute bottom-6 left-6 z-40 flex items-end gap-3">
        <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700 p-1.5 rounded-lg shadow-lg flex flex-col items-center gap-1">
          <button onClick={() => setZoom((z) => Math.min(2, z + 0.1))} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md text-slate-600 dark:text-slate-300 transition-colors" title="Zoom In (Ctrl +)"><ZoomIn size={20} /></button>
